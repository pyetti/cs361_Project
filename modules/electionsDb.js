...
var getCandidateInfoSql = "SELECT e.election_id, c.first_name, c.last_name, p.name as 'Party' FROM candidates c JOIN elections e ON c.election_id = e.election_id JOIN locations l ON e.location_id = l.location_id JOIN politicalParties p ON p.politcal_party_id = c.politcal_party_id WHERE l.zipcode = ?;";
var getPropositionInfoSql = "SELECT p.proposition_id, p.title, p.description, DATE_FORMAT(p.date_of_vote, '%m/%d/%Y') as 'date' FROM propositions p JOIN locations l ON p.location_id = l.location_id WHERE l.zipcode = ? ORDER BY p.proposition_id;";
...
var getCandidatesByElectionSql = "SELECT e.election_id, c.first_name, c.last_name, p.name as 'party' FROM candidates c JOIN elections e ON c.election_id = e.election_id JOIN locations l ON e.location_id = l.location_id JOIN politicalParties p ON p.politcal_party_id = c.politcal_party_id WHERE e.election_id = ? ORDER BY c.last_name;";
var getPropositionDetailsSql = "SELECT p.proposition_id, p.title, p.description, DATE_FORMAT(p.date_of_vote, '%m/%d/%Y') as 'date' FROM propositions p JOIN locations l ON p.location_id = l.location_id WHERE p.proposition_id = ?;";
